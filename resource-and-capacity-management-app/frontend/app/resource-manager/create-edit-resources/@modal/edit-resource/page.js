...
export default function EditResourceModal() {
  ...
  const handleEdit = async (e) => {
    ...
    // Duplicate name check — ensure no other employee has the same name
    try {
      const checkRes = await fetch(`${API_BASE}/resources/employees`);
      if (checkRes.ok) {
        const existing = await checkRes.json();
        const nameTaken = existing.some(
          e => e.emp_name?.toLowerCase().trim() === formData.emp_name.toLowerCase().trim() &&
               String(e.emp_id) !== String(empId)
        );
        if (nameTaken) return setError(`An employee named "${formData.emp_name.trim()}" already exists. Please use a different name.`);
      }
    } catch {

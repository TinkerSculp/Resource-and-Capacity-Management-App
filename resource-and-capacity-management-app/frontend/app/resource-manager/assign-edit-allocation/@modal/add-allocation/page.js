...
function SearchableStyledDropdown({ label, value, onChange, options, valueKey, displayKey }) {
  ...
  return (
    <div className="flex flex-col relative" ref={ref}>
      <label className="text-xs text-black mb-1">{label}</label>
      <div role="button" tabIndex={0} aria-haspopup="listbox" aria-expanded={open} onKeyDown={e => e.key === "Enter" && setOpen(!open)}
      ...
      {open && (
        <div role="listbox" className="absolute top-full left-0 w-full bg-white border border-gray-500 rounded shadow-lg z-50 mt-1">
          ...
          <div className="max-h-60 overflow-y-auto" role="group">
            ...
            {filtered.map(opt => (
              <div key={opt[valueKey]} role="option" aria-selected={String(opt[valueKey]) === String(value)}
...
export default function AddAllocationModal() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const alreadyExists = (dupRes.data?.allAssignments || []).some(r => String(r.employee?.emp_id) === String(selectedEmployee) && r.assignment?.project_name === selectedProject);
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[9999] px-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-4 sm:p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-end gap-3 mt-6">
          ...
          <button type="button" onClick={handleSave} disabled={loading} aria-disabled={loading} className={`${btnClass} bg-[#017ACB] text-white disabled:opacity-50 disabled:cursor-not-allowed`}>{loading ? "Saving..." : "Save Allocation"}</button>
        </div>
      </div>
    </div>
  );

...
function SearchableStyledDropdown({ label, value, onChange, options, valueKey, displayKey }) {
  ...
  return (
    <div className="flex flex-col relative" ref={ref}>
      <label className="text-xs text-black mb-1">{label}</label>
      <div className="bg-white text-black border border-gray-500 p-2 rounded cursor-pointer flex justify-between items-center hover:bg-[#017ACB]/20 transition" onClick={() => setOpen(!open)}>
        <span className="truncate">{selectedLabel || `Select ${label}`}</span>
      ...
      {open && (
        <div className="absolute top-full left-0 w-full bg-white border border-gray-500 rounded shadow-lg z-50 mt-1">
          <input type="text" placeholder="Search..." value={search} onChange={e => setSearch(e.target.value.replace(/[^a-zA-Z ]/g, ""))} className="p-2 border-b border-gray-500 w-full text-black bg-white hover:bg-[#017ACB]/20 transition focus:outline-none" />
          <div className="max-h-60 overflow-y-auto">
            {filtered.length === 0 && <div className="p-2 text-sm text-gray-400">No results</div>}
            {filtered.map(opt => (
              <div key={opt[valueKey]} className={`p-2 cursor-pointer text-sm text-black hover:bg-[#017ACB]/20 transition ${String(opt[valueKey]) === String(value) ? "bg-[#017ACB]/10 font-medium" : ""}`}
                onClick={() => { const raw = opt[valueKey]; onChange(typeof raw === 'number' || (raw !== null && !isNaN(Number(raw)) && raw !== '') ? Number(raw) : raw); setOpen(false); setSearch(""); }}>
                {opt[displayKey]}
              </div>
            ))}
          </div>
...
export default function EditAllocationModal() {
  ...
  async function handleSave() {
    if (!selectedProject || !selectedEmployee) { setError("Please select both a project and an employee."); return; }
    const oldEmpId = Number(emp_id), newEmpId = Number(selectedEmployee);
    const oldProject = projectFromURL, newProject = selectedProject;
    ...
    try {
      ...
      const alreadyExists = (dupRes.data?.allAssignments || []).some(r => String(r.employee?.emp_id) === String(newEmpId) && r.assignment?.project_name === newProject);
    ...
    try {
      await api.post('/assignments-allocations/reassign', {
        old_emp_id: oldEmpId, new_emp_id: newEmpId,
        old_project: oldProject, new_project: newProject,
        old_category: originalAssignment?.category || null,
        new_category: assignmentData?.category || null,
      });
      setSuccess(true);
      setTimeout(() => { router.back(); setTimeout(() => router.replace(`/resource-manager/assign-edit-allocation?refresh=${Date.now()}`), 120); }, 1200);
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
        <div className="bg-white text-black p-6 rounded-lg border border-black shadow-xl w-full max-w-md">Loading…</div>
      </div>
    );
  }
    ...
    return (
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
        <div className="bg-white text-black p-6 rounded-lg border border-black shadow-xl w-full max-w-md">
          <h2 className="text-xl font-bold mb-4">Error</h2>
          <p className="text-sm">{error || "Unable to load allocation"}</p>
          <button className={`mt-4 ${btnClass} bg-[#003A5C] text-white`} onClick={() => router.back()}>Close</button>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {[
            ...
            ["Activity Category",  assignmentData?.category || "—"],
            ["Leader Accountable", assignmentData?.leader || "—"],
            ["Requestor",          assignmentData?.requestor || "—"],
            ["Requestor VP",       assignmentData?.requestor_vp || "—"],
            ["Requesting Dept",    assignmentData?.requesting_dept || "—"],
          ].map(([label, val]) => (
            <div key={label} className="flex flex-col">
              <label className="text-xs text-black mb-1">{label}</label>
              <div className="bg-gray-200 text-black border border-gray-500 p-2">{val}</div>
            </div>
          ))}
        </div>
        <div className="flex flex-col sm:flex-row sm:justify-end gap-3 mt-6">
          <button type="button" onClick={() => router.back()} className={`${btnClass} bg-[#003A5C] text-white`}>Close</button>
          <button type="button" onClick={handleSave} disabled={loading} aria-disabled={loading} className={`${btnClass} bg-[#017ACB] text-white disabled:opacity-50 disabled:cursor-not-allowed`}>Save Changes</button>
        </div>
      </div>
    </div>
  );

...
//               <button onClick={handleConfirmDelete} className="px-4 py-2 rounded text-sm bg-[#017ACB] text-white border border-black/50 dark:border-slate-500/60 hover:bg-[#017ACB]/80 dark:shadow-[4px_4px_10px_rgba(0,0,0,0.45)] dark:before:shadow-[inset_0_1px_2px_rgba(255,255,255,0.08),inset_0_-1px_2px_rgba(0,0,0,0.45)] transition shadow-[4px_4px_10px_rgba(0,0,0,0.25)] relative before:content-[''] before:absolute before:inset-0 before:rounded before:pointer-events-none before:shadow-[inset_0_1px_2px_rgba(255,255,255,0.22),inset_0_-1px_2px_rgba(0,0,0,0.15)]" style={styles.outfitFont}>Yes</button>
//             </div>
//           </div>
//         </div>
//       )}
...
//             </p>
//             <div className="flex justify-end gap-3">
//               <button onClick={() => setOverAllocConfirm(null)} className="px-4 py-2 rounded text-sm bg-[#003A5C] dark:bg-[#0A5F8A] text-white border border-black/50 dark:border-slate-500/60 hover:bg-[#017ACB]/20 dark:hover:bg-[#017ACB]/30 transition shadow-[4px_4px_10px_rgba(0,0,0,0.25)] relative before:content-[''] before:absolute before:inset-0 before:rounded before:pointer-events-none before:shadow-[inset_0_1px_2px_rgba(255,255,255,0.22),inset_0_-1px_2px_rgba(0,0,0,0.15)]" style={styles.outfitFont}>No</button>
...
/* =============================================================================
   ...
   -----------------------------------------------------------------------------
   PURPOSE:
     Displays all employee assignments and their monthly FTE allocations in a
     filterable, horizontally scrollable table. Supports:
       • "All Assignments" and "My Assignments" tab views
       • Column filter dropdowns (resource, project, category, leader,
         requestor, requestor VP, requesting dept, reports to)
       • Resource name sort (A→Z / Z→A) and inline search
       • Start month selector — shows 16 months from the chosen month
       • Inline allocation editing — click a cell to edit, blur/enter to save
       • Over-allocation warning dialog — shown when a save would exceed capacity
       • Last-allocation removal confirmation dialog

   HOW IT WORKS:
     1. On mount, reads the user session from localStorage
     2. Fetches allAssignments and myAssignments from the backend in one call
     3. Defaults startMonth to the current calendar month
     4. Inline edits PUT to /assignments-allocations/:id/amount
     5. Clearing the last allocation for a row triggers a DELETE with a
   ...
   SECURITY MODEL:
     ...
     • username passed through encodeURIComponent() in the API URL.
     • emp_id, project, and category passed through encodeURIComponent() in
       the edit navigation URL.
     • All numeric allocation values validated before sending to backend.
     • Over-allocation check fetches capacity from the backend — not hardcoded.
     • Filter option lists built from server response data only.
...
/* -----------------------------------------------------------------------------
   ...
   isActive = false → Grey (gray-200) — muted, available to click
     → Hover darkens to gray-300 — subtle feedback
----------------------------------------------------------------------------- */
const tabClass = (isActive) => `
  px-4 py-2 rounded text-sm
  border border-[#00263F]/50 dark:border-slate-500/60

      ...
      {open && (
        <div className="absolute top-full left-0 right-0 bg-white dark:bg-[#1f1f1f] border border-black dark:border-slate-600 rounded mt-1 z-50 shadow-lg dark:shadow-[0_10px_30px_rgba(0,0,0,0.45)]">
          ...
          <div className="max-h-40 overflow-y-auto">
            {filtered.map(emp => (
              <div key={emp.emp_name} onClick={() => { onChange(emp.emp_name); setOpen(false); setSearch(''); }} className={`p-2 cursor-pointer text-black dark:text-slate-100 hover:bg-[#017ACB]/20 dark:hover:bg-[#017ACB]/30 transition text-sm ${value === emp.emp_name ? 'font-bold bg-[#CDE6F7] dark:bg-[#0A5F8A]/40' : ''}`} style={styles.outfitFont}>{emp.emp_name}</div>
            ))}
      ...
      <div className="bg-white dark:bg-[#212121] rounded-lg shadow-xl dark:shadow-[0_12px_40px_rgba(0,0,0,0.55)] border border-transparent dark:border-slate-700 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-2xl font-bold text-black dark:text-white" style={styles.outfitFont}>Add Initiative</h2>
            <button
              type="button"
              onClick={() => router.back()}
              disabled={loading}
              aria-label="Close"
              className="text-gray-500 dark:text-slate-400 hover:text-black dark:hover:text-white transition text-2xl font-bold leading-none px-2 py-1 rounded hover:bg-gray-100 dark:hover:bg-slate-700"

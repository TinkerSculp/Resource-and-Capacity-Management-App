...
// /* -----------------------------------------------------------------------------
//    BLOCKED WORDS — checked on submit for description and resource consideration
// ----------------------------------------------------------------------------- */
// const BLOCKED_WORDS = [
...
//     if (containsBlockedWords(form.description))
//       return setError('Description contains inappropriate language. Please revise.');
//     if (containsBlockedWords(form.resource_consideration))
//       return setError('Resource Consideration contains inappropriate language. Please revise.');
...
//               {/* TARGET PERIOD — letters, numbers, spaces, common punctuation + / */}
//               <div className="flex flex-col">
...
export default function AddInitiativeModal() {
  ...
  const handleSubmit = async (e) => {
    ...
    if ((form.status === 'Completed' || form.status === 'Cancelled') && !form.completion_date)
      return setError('Completion date is required when status is Completed or Cancelled.');
    if (containsBlockedWords(form.project))
      return setError('Project Name contains inappropriate language. Please revise.');
    if (containsBlockedWords(form.target_period))
      return setError('Target Period contains inappropriate language. Please revise.');
    if (containsBlockedWords(form.description))
      return setError('Description contains inappropriate language. Please revise.');
    if (containsBlockedWords(form.resource_consideration))
      return setError('Resource Consideration contains inappropriate language. Please revise.');

...
/* =============================================================================
   ...
   TAB BUTTON COLOUR LOGIC:
     ...
     • Inactive (other tabs) = Grey (gray-200) — muted, available to click
...
/* -----------------------------------------------------------------------------
   SHARED BUTTON CLASSES — neumorphic, matches all other pages.
----------------------------------------------------------------------------- */
const btnClass = `
...
const Checkbox = ({ checked }) => (
  <span className="w-4 h-4 border border-black dark:border-slate-400 rounded-sm flex items-center justify-center transition relative overflow-hidden flex-shrink-0">
...
export default function InitiativesPage() {
  ...
  /* ---------------------------------------------------------------------------
     DERIVED: visibleStatuses — scoped to active tab
  --------------------------------------------------------------------------- */
  ...
  /* ---------------------------------------------------------------------------
     EFFECT 3: APPLY FILTERS + SORT + BUILD FILTER LISTS
  --------------------------------------------------------------------------- */
  useEffect(() => {
    if (!user) return;
          ...
          <table className="min-w-max w-full border-collapse">
            <thead className="bg-[#017ACB] text-white sticky top-0 z-[100]">
              <tr>
                ...
                {/* EDIT — sticky left */}
                <th className="sticky left-0 top-0 z-[9999] bg-[#017ACB] px-4 py-2 text-sm font-semibold whitespace-nowrap align-middle border-r border-black bg-clip-padding" style={styles.outfitFont}>
                  Edit
                </th>
            ...
            <tbody>
              {filteredInitiatives.length === 0 && (
                <tr>
                  <td colSpan={12} className="text-center py-8 text-gray-500 dark:text-slate-400 border border-black dark:border-slate-700" style={styles.outfitFont}>
              ...
              {filteredInitiatives.map(item => {
                ...
                return (
                  <tr
                    ...
                    className={`cursor-pointer transition-colors border-t border-black dark:border-slate-700 hover:bg-[#017ACB]/10 dark:hover:bg-[#017ACB]/20 ${isHighlighted ? 'bg-[#CDE6F7] dark:bg-[#0A5F8A]/30' : 'bg-white dark:bg-slate-900'}`}
                  >
                    {/* EDIT — sticky left */}
                    <td
                      className={`sticky left-0 z-30 px-4 py-2 border-r border-black dark:border-slate-700 whitespace-nowrap bg-clip-padding ${isHighlighted ? 'bg-[#CDE6F7] dark:bg-[#0A5F8A]/30' : 'bg-white dark:bg-slate-900'}`}

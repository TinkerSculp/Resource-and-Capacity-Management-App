...
/* =============================================================================
   ...
   HOW IT WORKS:
     ...
     5. On failure, a targeted error message is shown inline

   ROLE-BASED ROUTING:
     acc_type_id 1 → /resource-manager/dashboard
     acc_type_id 2 → /stakeholder/dashboard
     acc_type_id 3 → /team-member/dashboard
     acc_type_id 4 → /admin/dashboard
   ...
   SECURITY MODEL:
     ...
     • localStorage stores only the token string and a safe user object —
       the password is never written to localStorage under any circumstance.
     • Role-based routing is derived from acc_type_id returned by the backend —
       the client never decides the role, only acts on what the server returned.
     • The backend returns distinct error codes (username_not_found, wrong_password,
       account_inactive) so the frontend can show targeted messages. This is an
       accepted usability trade-off for this internal application.
     • Username input strips non-alphanumeric characters on keystroke — prevents
       injection characters from being typed into the field.
     • Password input strips a minimal set of injection characters (<>'"`;) —
       complex passwords with ! @ # $ % etc. are still allowed.

   FORGOT PASSWORD:
     The Forgot Password link opens an inline modal explaining that the user
     should contact their administrator. The full forgot-password flow is
     implemented in ForgotPasswordPage.jsx but is not yet wired to email sending.

   RESPONSIVENESS:
     • m-4 on the card — edge padding on mobile.
     • w-full max-w-lg — fills narrow screens, capped at lg on desktop.
     • Header uses flex with flex-1 — logo and title scale without overflowing.
...
export default function LoginPage() {
  ...
  /* ---------------------------------------------------------------------------
     ...
     showForgotModal: Controls the inline forgot-password modal.
     loginError: Inline error message shown beneath the form on failure.
  --------------------------------------------------------------------------- */
  ...
  /* ---------------------------------------------------------------------------
     HANDLER: handleLogin
     ---------------------------------------------------------------------------
     Submits credentials, validates the response, stores the session, and
     routes the user to their role-specific dashboard.

     SECURITY:
     • e.preventDefault() — prevents GET submission, keeps credentials out of URL.
     • Credentials sent in POST body only — never as URL parameters.
     • Both user and token validated before storing — no partial session state.
     • localStorage stores only token and safe user fields — password never stored.
     • acc_type_id comes from the validated server response — client never assigns role.
  --------------------------------------------------------------------------- */
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Show targeted message based on backend error code — falls back to generic
  ...
  return (
    <>
      {/* BACKDROP — clicking outside the card stays on /login */}
      <div
      ...
      >
        ...
        <div
          className="bg-white rounded-xl shadow-xl p-6 sm:p-8 w-full max-w-lg m-4 border border-gray-200"
          onClick={(e) => e.stopPropagation()}
        >

          {/* HEADER: Logo + Company name + App name */}
          <div className="flex justify-between items-center mb-4 sm:mb-6">
            <Image src="/CapstoneDynamicsLogo.png" alt="Capstone Dynamics logo" width={96} height={96} className="h-16 w-16 sm:h-24 sm:w-24 shrink-0" priority />
            <div className="flex flex-col items-center flex-1 mx-3 sm:mx-4">
              <h3 className="text-lg sm:text-2xl font-bold text-[#017ACB]" style={{ fontFamily: 'Outfit, sans-serif' }}>Capstone Dynamics</h3>
              <h4 className="text-xs sm:text-base font-semibold text-black mt-1 text-center" style={{ fontFamily: 'Outfit, sans-serif' }}>Resource & Capacity Management</h4>
            </div>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4 sm:space-y-6">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-5 py-3 border text-gray-700 border-gray-300 rounded-lg text-base hover:bg-[#017ACB]/20 transition"
            ...
            {/* PASSWORD INPUT */}
            <div>
              <label htmlFor="password-input" className="block text-base font-medium text-gray-700 mb-2">Password</label>
              <div className="relative">
                <input
                  ...
                  value={password}
                  onChange={(e) => {
                    // Strip injection characters — allows complex passwords with ! @ # $ etc.
                    const sanitized = e.target.value.replace(/[<>'"`;]/g, '');
                    setPassword(sanitized);
                  }}
                  className="w-full px-5 py-3 pr-12 border text-gray-700 border-gray-300 rounded-lg text-base hover:bg-[#017ACB]/20 transition"
                  autoComplete="current-password"
                  maxLength={100}
                  required
                />
                {/* Show/hide toggle — type="button" prevents accidental form submission */}
                <button
                  type="button"
            ...
            {/* FORGOT PASSWORD LINK — opens inline modal */}
            ...
            {/* INLINE ERROR BANNER */}
            {loginError && (
              <div role="alert" className="p-3 bg-red-50 border border-red-300 text-red-700 rounded-lg text-sm flex items-start gap-2" style={{ fontFamily: 'Outfit, sans-serif' }}>
            ...
            <div className="flex gap-4">
              <button
                type="submit"
                className="
                  w-full sm:w-2/3 sm:mx-auto px-5 py-3 sm:py-3 border border-black/50
                  bg-[#017ACB] text-white rounded-lg text-sm sm:text-lg transition-all
      ...
      {/* FORGOT PASSWORD MODAL
          Explains that the user should contact their administrator.
      ...
      {showForgotModal && (
        ...
        >
          ...
          >
            ...
            <h3 className="text-lg font-bold text-black mb-2" style={{ fontFamily: 'Outfit, sans-serif' }}>Forgot Password?</h3>
            ...
            <button
              ...
              className="
                ...
                bg-[#017ACB] text-white rounded-lg text-sm

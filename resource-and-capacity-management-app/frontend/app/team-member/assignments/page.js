  ...
  const renderMenuItems = (available, selected, setSelected, sortOptions = false) => (
    <>
      {sortOptions && (
        <>
          {[{ val: "asc", label: "A → Z" }, { val: "desc", label: "Z → A" }].map(({ val, label }) => (
            <div key={val} className={`px-3 py-2 cursor-pointer text-sm flex items-center gap-2 hover:bg-[#017ACB]/20 dark:hover:bg-[#017ACB]/30 ${resourceSort === val ? "font-bold" : ""}`} onClick={() => setResourceSort(resourceSort === val ? "" : val)}>
              <Checkbox checked={resourceSort === val} />{label}
            </div>
          ))}
          <div className="border-t my-1 dark:border-slate-600" />
      ...
      {available.map(val => (
        <div key={val} className={`px-3 py-2 cursor-pointer text-sm flex items-center gap-2 hover:bg-[#017ACB]/20 dark:hover:bg-[#017ACB]/30 ${selected.includes(val) ? "font-bold" : ""}`} onClick={() => toggleSelection(val, setSelected, selected)}>
          <Checkbox checked={selected.includes(val)} />{val}
        </div>
      ))}
    </>
          ...
          <table className="min-w-max w-full border-collapse text-sm">
            <thead className="bg-[#017ACB] text-white sticky top-0 z-[100]">
              <tr>
                ...
                {showMonthMenu && (
                  <div ref={monthMenuRef} className={menuClass} style={{ position: "fixed", top: menuPosition.y, left: menuPosition.x }} onClick={e => e.stopPropagation()}>
                    {[...availablePastMonths].reverse().map(m => (
                      <div key={m} data-month={m} className={`px-3 py-2 cursor-pointer text-sm flex items-center gap-2 hover:bg-[#017ACB]/20 dark:hover:bg-[#017ACB]/30 ${startMonth === m ? "font-bold" : ""}`} onClick={() => setStartMonth(m)}>
            ...
            <tbody>
              {filteredRows.length === 0 ? (
                ...
                <tr key={index} className={`group transition-colors hover:bg-[#017ACB]/10 dark:hover:bg-[#017ACB]/20 border-t border-black dark:border-slate-700 ${index % 2 === 0 ? "bg-white dark:bg-slate-900" : "bg-gray-50 dark:bg-slate-800/60"}`}>
                  <td className={`sticky left-0 z-20 px-4 py-2 border border-black dark:border-slate-700 text-sm text-black dark:text-slate-100 whitespace-nowrap min-w-[150px] ${index % 2 === 0 ? "bg-white dark:bg-slate-900" : "bg-gray-50 dark:bg-slate-800/60"} group-hover:bg-[#017ACB]/10 dark:group-hover:bg-[#017ACB]/20`} style={styles.outfitFont}>{row.resource_name}</td>
                  <td className="px-4 py-2 border border-black dark:border-slate-700 text-sm text-black dark:text-slate-100 whitespace-nowrap" style={styles.outfitFont}>{row.department}</td>
                  <td className="px-4 py-2 border border-black dark:border-slate-700 text-sm text-black dark:text-slate-100 whitespace-nowrap" style={styles.outfitFont}>{row.reports_to}</td>
                  <td className="px-4 py-2 border border-black dark:border-slate-700 text-sm text-black dark:text-slate-100 whitespace-nowrap" style={styles.outfitFont}>{row.activity}</td>
                  <td className="px-4 py-2 border border-black dark:border-slate-700 text-sm text-black dark:text-slate-100 whitespace-nowrap" style={styles.outfitFont}>{row.category}</td>
                  <td className="px-4 py-2 border border-black dark:border-slate-700 text-sm text-black dark:text-slate-100 whitespace-nowrap" style={styles.outfitFont}>{row.leader}</td>
                  <td className="px-4 py-2 border border-black dark:border-slate-700 text-sm text-black dark:text-slate-100 whitespace-nowrap" style={styles.outfitFont}>{row.requestor}</td>
                  <td className="px-4 py-2 border border-black dark:border-slate-700 text-sm text-black dark:text-slate-100 whitespace-nowrap" style={styles.outfitFont}>{row.requestor_vp}</td>
                  <td className="px-4 py-2 border border-black dark:border-slate-700 text-sm text-black dark:text-slate-100 whitespace-nowrap" style={styles.outfitFont}>{row.requesting_dept}</td>
                  {visibleMonths.map(m => (
                    <td key={m} className="px-2 py-2 border border-black dark:border-slate-700 text-sm text-black dark:text-slate-100 text-center whitespace-nowrap" style={styles.outfitFont}>{row.allocations[m] ?? ""}</td>
                  ))}
                </tr>
              ))}
            </tbody>

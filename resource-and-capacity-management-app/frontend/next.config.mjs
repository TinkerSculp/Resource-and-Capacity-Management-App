...
//      that the Express backend serves as static files in production.
//   --------------------------------------------------------------------------- */
//   output: "standalone",
    ...
    return [
      {
        ...
        headers: [
          ...
          /* -----------------------------------------------------------------
             Referrer-Policy: strict-origin-when-cross-origin
             Controls what is included in the Referer header when navigating
             to another origin. Prevents sensitive URL parameters from leaking
             to third-party analytics or ad networks.
          ----------------------------------------------------------------- */
          { key: "Referrer-Policy", value: "strict-origin-when-cross-origin" },

          /* -----------------------------------------------------------------
             Permissions-Policy
             Explicitly disables browser APIs this application does not use.
             camera=()      — No access to the device camera
             microphone=()  — No access to the microphone
             geolocation=() — No access to GPS or location data
          ----------------------------------------------------------------- */
